 ...
 */
package com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.client;
...
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.datasource.entity.SentinelVersion;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.datasource.entity.gateway.ApiDefinitionEntity;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.datasource.entity.gateway.GatewayFlowRuleEntity;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.datasource.entity.rule.*;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.discovery.AppManagement;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.domain.cluster.ClusterClientInfoVO;
...
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.domain.cluster.state.ClusterServerStateVO;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.domain.cluster.state.ClusterStateSimpleEntity;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.util.AsyncUtils;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.util.VersionUtils;
...
public class SentinelApiClient {
    ...
    private CompletableFuture<String> executeCommand(String app, String ip, int port, String api, Map<String, String> params, boolean useHttpPost) {
        ...
        boolean supportPost =
                StringUtil.isNotEmpty(app) && Optional.ofNullable(appManagement.getDetailApp(app)).flatMap(e -> e.getMachine(ip, port)).flatMap(m -> VersionUtils.parseVersion(m.getVersion()).map(v -> v.greaterOrEqual(version160))).orElse(false);
    ...
    public CompletableFuture<List<ParamFlowRuleEntity>> fetchParamFlowRulesOfMachine(String app, String ip, int port) {
        try {
            ...
            AssertUtil.notEmpty(ip, "Bad machine IP");
            AssertUtil.isTrue(port > 0, "Bad machine port");
            return fetchItemsAsync(ip, port, GET_PARAM_RULE_PATH, null, ParamFlowRule.class).thenApply(rules -> rules.stream().map(e -> ParamFlowRuleEntity.fromAuthorityRule(app, ip, port, e)).collect(Collectors.toList()));
        }
        catch (Exception e) {
    ...
    public CompletableFuture<ClusterStateSimpleEntity> fetchClusterMode(String ip, int port) {
        ...
        try {
            return executeCommand(ip, port, FETCH_CLUSTER_MODE_PATH, false).thenApply(r -> JSON.parseObject(r, ClusterStateSimpleEntity.class));
        }
        catch (Exception ex) {
    ...
    public CompletableFuture<ClusterClientInfoVO> fetchClusterClientInfoAndConfig(String ip, int port) {
        ...
        try {
            return executeCommand(ip, port, FETCH_CLUSTER_CLIENT_CONFIG_PATH, false).thenApply(r -> JSON.parseObject(r, ClusterClientInfoVO.class));
        }
        catch (Exception ex) {
    ...
    public CompletableFuture<Void> modifyClusterServerFlowConfig(String app, String ip, int port, ServerFlowConfig config) {
        ...
        try {
            Map<String, String> params = new HashMap<>(1);
            params.put("data", JSON.toJSONString(config));
            return executeCommand(app, ip, port, MODIFY_CLUSTER_SERVER_FLOW_CONFIG_PATH, params, true).thenCompose(e -> {
                if (CommandConstants.MSG_SUCCESS.equals(e)) {
                    return CompletableFuture.completedFuture(null);
                }
                else {
    ...
    public CompletableFuture<Void> modifyClusterServerTransportConfig(String app, String ip, int port, ServerTransportConfig config) {
        ...
        try {
            ...
            return executeCommand(app, ip, port, MODIFY_CLUSTER_SERVER_TRANSPORT_CONFIG_PATH, params, false).thenCompose(e -> {
                if (CommandConstants.MSG_SUCCESS.equals(e)) {
                    return CompletableFuture.completedFuture(null);
                }
                else {
    ...
    public CompletableFuture<Void> modifyClusterServerNamespaceSet(String app, String ip, int port, Set<String> set) {
        ...
        try {
            ...
            return executeCommand(app, ip, port, MODIFY_CLUSTER_SERVER_NAMESPACE_SET_PATH, params, true).thenCompose(e -> {
                if (CommandConstants.MSG_SUCCESS.equals(e)) {
                    return CompletableFuture.completedFuture(null);
                }
                else {
    ...
    public CompletableFuture<ClusterServerStateVO> fetchClusterServerBasicInfo(String ip, int port) {
        ...
        try {
            return executeCommand(ip, port, FETCH_CLUSTER_SERVER_BASIC_INFO_PATH, false).thenApply(r -> JSON.parseObject(r, ClusterServerStateVO.class));
        }
        catch (Exception ex) {
