 ...
 */
package com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.controller.cluster;

import com.alibaba.csp.sentinel.cluster.ClusterStateManager;
import com.alibaba.csp.sentinel.util.StringUtil;
import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.client.CommandNotFoundException;
...
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.domain.Result;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.domain.cluster.request.ClusterClientModifyRequest;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.domain.cluster.request.ClusterModifyRequest;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.domain.cluster.request.ClusterServerModifyRequest;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.domain.cluster.state.AppClusterClientStateWrapVO;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.domain.cluster.state.AppClusterServerStateWrapVO;
...
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.service.ClusterConfigService;
...
public class ClusterConfigController {
    ...
    public Result<List<AppClusterServerStateWrapVO>> apiGetClusterServerStateOfApp(@PathVariable String app) {
        ...
        try {
            return clusterConfigService.getClusterUniversalState(app).thenApply(ClusterEntityUtils::wrapToAppClusterServerState).thenApply(Result::ofSuccess).get();
        }
        catch (ExecutionException ex) {
    ...
    public Result<List<AppClusterClientStateWrapVO>> apiGetClusterClientStateOfApp(@PathVariable String app) {
        ...
        try {
            return clusterConfigService.getClusterUniversalState(app).thenApply(ClusterEntityUtils::wrapToAppClusterClientState).thenApply(Result::ofSuccess).get();
        }
        catch (ExecutionException ex) {
    ...
    private boolean checkIfSupported(String app, String ip, int port) {
        try {
            return Optional.ofNullable(appManagement.getDetailApp(app)).flatMap(e -> e.getMachine(ip, port)).flatMap(m -> VersionUtils.parseVersion(m.getVersion()).map(v -> v.greaterOrEqual(version140))).orElse(true);
