...
/**
 * 自定义MetricFetcher,需要注释掉默认的:MetricFetcher
 */
@Component
public class CustomizedMetricFetcher implements InitializingBean, NodeChangedListener {
    ...
    private static Logger logger = LoggerFactory.getLogger(CustomizedMetricFetcher.class);
    ...
    private ScheduledExecutorService fetchScheduleService = Executors.newScheduledThreadPool(1, new NamedThreadFactory("sentinel-dashboard-metrics-fetch-task"));
    private ExecutorService fetchService;
    private ExecutorService fetchWorker;

    //全局数据采集开关
    private volatile boolean closeDataCollect;

    //当前节点是否为最老节点（只有最老节点才能够采集数据）
    private volatile boolean isMasterNode;

    public CustomizedMetricFetcher() {
        int cores = Runtime.getRuntime().availableProcessors() * 2;
        long keepAliveTime = 0;
        int queueSize = 2048;
        RejectedExecutionHandler handler = new DiscardPolicy();
        fetchService = new ThreadPoolExecutor(cores, cores, keepAliveTime, TimeUnit.MILLISECONDS, new ArrayBlockingQueue<>(queueSize), new NamedThreadFactory("sentinel-dashboard-metrics-fetchService"), handler);
        fetchWorker = new ThreadPoolExecutor(cores, cores, keepAliveTime, TimeUnit.MILLISECONDS, new ArrayBlockingQueue<>(queueSize), new NamedThreadFactory("sentinel-dashboard-metrics-fetchWorker"), handler);
        ...
        //不能再构造函数中启动，因为构造的过程中会导致Ignite的启动(appManagement)，这个比较耗时间，
        // 所以会导致线程运行的时候实例还没有真正启动,从而出错。因此需要在afterPropertiesSet中启动
        //        httpclient.start();
        //        start();
    }

    public void setCloseDataCollect(boolean closeDataCollect) {
        this.closeDataCollect = closeDataCollect;
    }
    ...
    private void start() {
        fetchScheduleService.scheduleAtFixedRate(() -> {
            try {
                if (noDataCollect()) {
                    logger.debug("Data collect has been closed.");
                    return;
                }

                logger.debug("Try to fetch data from apps.");
    ...
    //只有全局开关没有关闭，且是主节点才能够采集数据
    private boolean noDataCollect() {
        return closeDataCollect || !isMasterNode;
    }
    ...
    private void fetchAllApp() {
        ...
        if (apps == null) {
            logger.warn("There is no valid apps to be fetched data.");
            return;
        }
    ...
    private void fetchOnce(String app, long startTime, long endTime, int maxWaitSeconds) {
        ...
        logger.info("finished " + msg + " metric for " + app + ", time intervalMs [" + startTime + ", " + endTime + "], total machines=" + machines.size() + ", fetch success=" + success + ", fetch fail=" + fail + ", time cost=" + cost
                    + " ms, metricMap size:" + metricMap.size());
    ...
    private void handleResponse(final HttpResponse response, MachineInfo machine, Map<String, MetricEntity> metricMap) throws Exception {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(machine.getApp() + ":" + machine.getIp() + ":" + machine.getPort() + ", bodyStr.length()=" + body.length() + ", lines=" + lines.length);// + ", resp:" + body);
        }
    ...
    public void nodeChanged(DiscoveryEvent evt, Boolean isMasterChanged) {
        if (null == isMasterChanged) {
            return;
        }
        setMasterNode(isMasterChanged);
    }
}