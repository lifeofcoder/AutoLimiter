 ...
 */
package com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.util;

import com.alibaba.csp.sentinel.cluster.ClusterStateManager;
import com.alibaba.csp.sentinel.util.StringUtil;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.domain.cluster.ClusterGroupEntity;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.domain.cluster.ConnectionGroupVO;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.domain.cluster.state.*;
    ...
    public static List<AppClusterServerStateWrapVO> wrapToAppClusterServerState(List<ClusterUniversalStatePairVO> list) {
        ...
        for (ClusterUniversalStatePairVO stateVO : list) {
            ...
            if (mode == ClusterStateManager.CLUSTER_SERVER) {
                ...
                map.computeIfAbsent(serverId, v -> new AppClusterServerStateWrapVO().setId(serverId).setIp(ip).setPort(serverStateVO.getPort()).setState(serverStateVO).setBelongToApp(true)
                        .setConnectedCount(serverStateVO.getConnection().stream().mapToInt(ConnectionGroupVO::getConnectedCount).sum()));
