...
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.domain.cluster.ClusterAppFullAssignRequest;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.domain.cluster.ClusterAppSingleServerAssignRequest;
import com.lifeofcoder.autolimiter.dashboard.sentinel.dashboard.service.ClusterAssignService;
...
public class ClusterAssignController {
    ...
    public Result<ClusterAppAssignResultVO> apiAssignAllClusterServersOfApp(@PathVariable String app, @RequestBody ClusterAppFullAssignRequest assignRequest) {
        ...
        try {
            return Result.ofSuccess(clusterAssignService.applyAssignToApp(app, assignRequest.getClusterMap(), assignRequest.getRemainingList()));
        }
    ...
    public Result<ClusterAppAssignResultVO> apiAssignSingleClusterServersOfApp(@PathVariable String app, @RequestBody ClusterAppSingleServerAssignRequest assignRequest) {
        ...
        try {
            return Result.ofSuccess(clusterAssignService.applyAssignToApp(app, Collections.singletonList(assignRequest.getClusterMap()), assignRequest.getRemainingList()));
        }
        catch (Throwable throwable) {
