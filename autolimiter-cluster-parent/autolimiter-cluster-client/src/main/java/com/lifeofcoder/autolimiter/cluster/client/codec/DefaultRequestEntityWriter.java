...
public class DefaultRequestEntityWriter implements RequestEntityWriter<ClusterRequest, ByteBuf> {
    private static Logger LOGGER = LoggerFactory.getLogger(DefaultRequestEntityWriter.class);
    ...
    public void writeTo(ClusterRequest request, ByteBuf target) {
        ...
        if (requestDataWriter == null) {
            LOGGER.warn("[DefaultRequestEntityWriter] Cannot find matching request writer for type <{}>," + " dropping the request", type);
            return;
        }
