...
public final class ClusterParamFlowRuleManager {
    private static Logger LOGGER = LoggerFactory.getLogger(ClusterParamFlowRuleManager.class);
    ...
    public static void register2Property(String namespace) {
        ...
        if (propertySupplier == null) {
            LOGGER.warn("[ClusterParamFlowRuleManager] Cluster param rule property supplier is absent, cannot register " + "property");
            return;
        }
        SentinelProperty<List<ParamFlowRule>> property = propertySupplier.apply(namespace);
        if (property == null) {
            LOGGER.warn("[ClusterParamFlowRuleManager] Wrong created property from cluster param rule property supplier, " + "ignoring");
            return;
        }
        synchronized (UPDATE_LOCK) {
            LOGGER.info("[ClusterParamFlowRuleManager] Registering new property to cluster param rule manager" + " for namespace <{}>", namespace);
    ...
    public static void removeProperty(String namespace) {
        ...
        synchronized (UPDATE_LOCK) {
            ...
            LOGGER.info("[ClusterParamFlowRuleManager] Removing property from cluster flow rule manager" + " for namespace <{}>", namespace);
        }
    }
    ...
    private static class ParamRulePropertyListener implements PropertyListener<List<ParamFlowRule>> {
        ...
        public void configLoad(List<ParamFlowRule> conf) {
            ...
            LOGGER.info("[ClusterParamFlowRuleManager] Cluster parameter rules loaded for namespace <{}>: {}", namespace, PARAM_RULES);
        }

        @Override
        public void configUpdate(List<ParamFlowRule> conf) {
            applyClusterParamRules(conf, namespace);
            LOGGER.info("[ClusterParamFlowRuleManager] Cluster parameter rules received for namespace <{}>: {}", namespace, PARAM_RULES);
        }
    }
    ...
    private static void applyClusterParamRules(List<ParamFlowRule> list, /*@Valid*/ String namespace) {
        ...
        for (ParamFlowRule rule : list) {
            ...
            if (!ParamFlowRuleUtil.isValidRule(rule)) {
                LOGGER.warn("[ClusterParamFlowRuleManager] Ignoring invalid param flow rule when loading new flow rules: " + rule);
                continue;
            }
