...
import com.lifeofcoder.autolimiter.cluster.server.util.ClusterRuleUtil;
...
public final class ClusterFlowRuleManager {
    private static Logger LOGGER = LoggerFactory.getLogger(ClusterFlowRuleManager.class);
    ...
    public static void register2Property(String namespace) {
        ...
        if (propertySupplier == null) {
            LOGGER.warn("[ClusterFlowRuleManager] Cluster flow property supplier is absent, cannot register property");
            return;
        }
        SentinelProperty<List<FlowRule>> property = propertySupplier.apply(namespace);
        if (property == null) {
            LOGGER.warn("[ClusterFlowRuleManager] Wrong created property from cluster flow property supplier, ignoring");
            return;
        }
        synchronized (UPDATE_LOCK) {
            LOGGER.info("[ClusterFlowRuleManager] Registering new property to cluster flow rule manager" + " for namespace <{}>", namespace);
    ...
    public static void removeProperty(String namespace) {
        ...
        synchronized (UPDATE_LOCK) {
            ...
            LOGGER.info("[ClusterFlowRuleManager] Removing property from cluster flow rule manager" + " for namespace <{}>", namespace);
        }
    }
    ...
    private static void applyClusterFlowRule(List<FlowRule> list, /*@Valid*/ String namespace) {
        ...
        final Map<Long, FlowRule> ruleMap = FLOW_RULES;

        Set<Long> flowIdSet = NAMESPACE_FLOW_ID_MAP.computeIfAbsent(namespace, v -> new HashSet<>());
        for (FlowRule rule : list) {
            if (!rule.isClusterMode()) {
                continue;
            }
            if (!FlowRuleUtil.isValidRule(rule)) {
                LOGGER.warn("[ClusterFlowRuleManager] Ignoring invalid flow rule when loading new flow rules: " + rule);
                continue;
            }
            ...
            int maxCount = (int) rule.getCount();
            // Prepare cluster metric from valid flow ID.
            ClusterMetric newClusterMetric = ClusterMetricStatistics.computeMetricIfAbsent(flowId, new ClusterMetric(flowId, clusterConfig.getSampleCount(), clusterConfig.getWindowIntervalMs(), maxCount));
            newClusterMetric.tryUpdateMaxCount(maxCount);
        }
    ...
    public static void deleteFlowRule(Long flowId) {
        FLOW_RULES.remove(flowId);
        String namespace = FLOW_NAMESPACE_MAP.remove(flowId);
        if (ValidatorHelper.isNotEmpty(namespace)) {
            Set<Long> flowIdSet = NAMESPACE_FLOW_ID_MAP.get(namespace);
            if (ValidatorHelper.isNotEmpty(flowIdSet)) {
                flowIdSet.remove(flowId);
            }
        }
        ClusterMetricStatistics.removeMetric(flowId);
    }
    ...
    private static final class FlowRulePropertyListener implements PropertyListener<List<FlowRule>> {
        ...
        public synchronized void configUpdate(List<FlowRule> conf) {
            ...
            LOGGER.info("[ClusterFlowRuleManager] Cluster flow rules received for namespace <{}>: {}", namespace, FLOW_RULES);
        }

        @Override
        public synchronized void configLoad(List<FlowRule> conf) {
            applyClusterFlowRule(conf, namespace);
            LOGGER.info("[ClusterFlowRuleManager] Cluster flow rules loaded for namespace <{}>: {}", namespace, FLOW_RULES);
        }
    }
