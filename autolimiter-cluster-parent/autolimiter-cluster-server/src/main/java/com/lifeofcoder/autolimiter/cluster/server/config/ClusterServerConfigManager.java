...
import com.lifeofcoder.autolimiter.cluster.flow.statistic.ClusterParamMetricStatistics;
import com.lifeofcoder.autolimiter.cluster.flow.statistic.limit.GlobalRequestLimiter;
import com.lifeofcoder.autolimiter.cluster.server.ServerConstants;
...
public final class ClusterServerConfigManager {
    private static Logger LOGGER = LoggerFactory.getLogger(ClusterServerConfigManager.class);
    ...
    private static volatile ServerTransportConfig config;
    ...
    public static void registerNamespaceSetProperty(SentinelProperty<Set<String>> property) {
        ...
        synchronized (NAMESPACE_SET_PROPERTY_LISTENER) {
            LOGGER.info("[ClusterServerConfigManager] Registering new namespace set dynamic property to Sentinel server " + "config manager");
    ...
    public static void registerServerTransportProperty(SentinelProperty<ServerTransportConfig> property) {
        ...
        synchronized (TRANSPORT_PROPERTY_LISTENER) {
            LOGGER.info("[ClusterServerConfigManager] Registering new server transport dynamic property to Sentinel server " + "config manager");
    ...
    public static void registerGlobalServerFlowProperty(SentinelProperty<ServerFlowConfig> property) {
        ...
        synchronized (GLOBAL_FLOW_PROPERTY_LISTENER) {
            LOGGER.info("[ClusterServerConfigManager] Registering new server global flow dynamic property " + "to Sentinel server config manager");
    ...
    private static class ServerNamespaceSetPropertyListener implements PropertyListener<Set<String>> {
        ...
        public synchronized void configLoad(Set<String> set) {
            if (set == null || set.isEmpty()) {
                LOGGER.warn("[ClusterServerConfigManager] WARN: empty initial server namespace set");
                return;
            }
    ...
    private static void applyNamespaceSetChange(Set<String> newSet) {
        ...
        LOGGER.info("[ClusterServerConfigManager] Server namespace set will be update to: " + newSet);
        if (newSet.isEmpty()) {
    ...
    private static class ServerGlobalTransportPropertyListener implements PropertyListener<ServerTransportConfig> {
        ...
        public void configLoad(ServerTransportConfig config) {
            if (config == null) {
                LOGGER.warn("[ClusterServerConfigManager] Empty initial server transport config");
                return;
            }
            applyConfig(config);
        }
        ...
        private synchronized void applyConfig(ServerTransportConfig config) {
            if (!isValidTransportConfig(config)) {
                LOGGER.warn("[ClusterServerConfigManager] Invalid cluster server transport config, ignoring: " + config);
                return;
            }
            LOGGER.info("[ClusterServerConfigManager] Updating new server transport config: " + config);
    ...
    private static void updateTokenServer(ServerTransportConfig newConfig) {
        if (Objects.equals(config, newConfig)) {
            LOGGER.info("Don't update token server because the config is not changed.");
            return;
        }

        int newPort = newConfig.getPort();
        ...
        for (ServerTransportConfigObserver observer : TRANSPORT_CONFIG_OBSERVERS) {
            observer.onTransportConfigChange(newConfig);
        }
    ...
    private static class ServerGlobalFlowPropertyListener implements PropertyListener<ServerFlowConfig> {
        ...
        private synchronized void applyGlobalFlowConfig(ServerFlowConfig config) {
            if (!isValidFlowConfig(config)) {
                LOGGER.warn("[ClusterServerConfigManager] Invalid cluster server global flow config, ignoring: " + config);
                return;
            }
            LOGGER.info("[ClusterServerConfigManager] Updating new server global flow config: " + config);
            ...
            if (newIntervalMs != intervalMs || newSampleCount != sampleCount) {
                if (newIntervalMs <= 0 || newSampleCount <= 0 || newIntervalMs % newSampleCount != 0) {
                    LOGGER.warn("[ClusterServerConfigManager] Ignoring invalid flow interval or sample count");
                }
                else {
    ...
    public static boolean isValidFlowConfig(ServerFlowConfig config) {
        return config != null && config.getMaxOccupyRatio() >= 0 && config.getExceedCount() >= 0 && config.getMaxAllowedQps() >= 0 && FlowRuleUtil.isWindowConfigValid(config.getSampleCount(), config.getIntervalMs());
    }
    ...
    public static ServerTransportConfig getConfig() {
        return config;
    }
